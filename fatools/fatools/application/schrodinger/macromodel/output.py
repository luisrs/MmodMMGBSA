...
class EnergyListingProteinResult(Parseable):
    def __init__(self, name, scoring_terms):
        self.name = name
        self.bound_ptn_total_energy = scoring_terms['bound_ptn_total_energy']
        self.bound_ptn_solvation_energy = scoring_terms['bound_ptn_solvation']

    @property
    def strain_protein(self):
        return self.bound_ptn_total_energy - self.bound_ptn_solvation_energy

    @classmethod
    def parser(cls):
        return EnergyListingProteinParser(cls)


class EnergyListingProteinParser(TextParser):
    ...
    def extract(self, output_file):
        ...
        for st in reader:
            if st.title not in ligand_dict:
                st.title.replace('ligand', 'protein')
                ligand_dict[st.title] = dict()
                ligand_dict[st.title]['bound_ptn_total_energy'] = (
                    st.property['r_mmod_Potential_Energy-OPLS-2005'])
                ligand_dict[st.title]['bound_ptn_solvation'] = (
                    st.property['r_mmod_Solvation_Energy-OPLS-2005'])

# ligand_dict {'1AQ1_ligand': {'bound_protein_energy': -22828.77734375}}

        for ligand in ligand_dict:
            list_ligands.append(EnergyListingProteinResult(
                ligand, ligand_dict[ligand]))
            self.ligands = tuple(list_ligands)
            # print "self.ligands", self.ligands
...
class RRHOEntropyResult(Parseable):

    def __init__(self, name, ts_translation, ts_rotation, ts_vibration, ts_total):
        self.name = name
        self.ts_translation = ts_translation
        self.ts_rotation = ts_rotation
        self.ts_vibration = ts_vibration
        self.ts_total = ts_total

    @classmethod
    def parser(cls):
        return RRHOEntropyParser(cls)


class RRHOEntropyParser(TextParser):
    ...
    def extract(self, csvfile):
        ...
        list_ligands = list()
        list_ligands.append(RRHOEntropyResult(
            energy_terms['Title'],
            energy_terms['TdS_trans'],
            energy_terms['TdS_rot'],
            energy_terms['TdS_vib'],
            energy_terms['TdS_total']))
        self.ligands = tuple(list_ligands)

# result = EnergyListingProteinResult.from_file('1AQ1_protein-out.mae')
# print "result.keys()", result.keys()[0]
# print result
# print result['1AQ1_protein'].bound_protein_energy

# result = RRHOEntropyResult.from_file('2EXM_entropyRRHO.csv')
# print result['2EXM_minimized_ligand'].ts_vibrational

# result = InteractionEnergyResult.from_file('1bcu.log')
# print result['1bcu_ligand'].electrostatic
